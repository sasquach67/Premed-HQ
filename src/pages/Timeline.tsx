      ...
      {milestones.map((m, i) => {
        ...
        return (
          <div key={m.id} className="relative animate-fade-up" style={{ animationDelay: `${i * 40}ms` }}>
            <span className={cn('absolute -left-[2.35rem] top-1 grid size-6 place-items-center rounded-full text-xs font-semibold ring-4 ring-background', past ? 'bg-muted text-muted-foreground' : soon ? 'bg-primary text-primary-foreground' : 'bg-secondary text-secondary-foreground')}>
              {i + 1}
            </span>
            <div className={cn('rounded-xl border border-border bg-card p-4 card-soft', soon && 'border-primary/40')}>
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h3 className="font-display text-lg font-semibold">{m.title}</h3>
                <span className={cn('rounded-full px-2.5 py-0.5 text-xs font-semibold', past ? 'bg-muted text-muted-foreground' : soon ? 'bg-primary/15 text-primary' : 'bg-secondary text-secondary-foreground')}>
