  ...
  return (
    <div className="space-y-3.5 pb-10">
      <PageHeader
        ...
        actions={<Button onClick={onAddEntity}><Plus className="size-4" /> {meta.add}</Button>}
      >
...
function SummaryKpi({ value, label, valueClassName }: { value: ReactNode; label: string; valueClassName?: string }) {
  return <span className="inline-flex items-baseline gap-2 whitespace-nowrap"><strong className={cn('font-display text-lg font-semibold tabular-nums', valueClassName)}>{value}</strong><span className="text-xs font-semibold uppercase tracking-[0.05em] text-muted-foreground">{label}</span></span>
}

function PillarSummary({ category }: { category: ExperienceLayoutProps['category'] }) {
  const shell = 'flex flex-wrap items-center gap-x-5 gap-y-2.5 rounded-xl border bg-card px-4 py-3 shadow-sm'
  if (category === 'volunteering') return (
    <section className={shell}>
      <SummaryKpi value="230" label="total hours" />
      <div className="flex flex-wrap items-center gap-2 text-xs font-semibold"><span className="text-[var(--cat-volunteer)]">Clinical 138</span><span className="flex h-2 w-32 overflow-hidden rounded-full bg-muted"><i className="h-full w-3/5 bg-[var(--cat-volunteer)]" /><i className="h-full w-2/5 bg-amber-500" /></span><span className="text-amber-600 dark:text-amber-300">Non-clin 92</span></div>
      <SummaryKpi value={<><span className="text-emerald-600 dark:text-emerald-300">138</span><span className="text-muted-foreground">/150</span></>} label="clin target" valueClassName="text-sm" />
      <SummaryKpi value={<><span className="text-emerald-600 dark:text-emerald-300">92</span><span className="text-muted-foreground">/100</span></>} label="non-clin target" valueClassName="text-sm" />
      <p className="text-xs font-semibold text-muted-foreground xl:ml-auto"><b className="text-foreground">3.4 hrs/wk</b> → both targets land by <b className="text-foreground">Feb 2027</b></p>
    </section>
  )
  if (category === 'shadowing') return (
    <section className={shell}>
      ...
      <p className="text-xs font-semibold text-muted-foreground xl:ml-auto"><span className="text-amber-600 dark:text-amber-300">Gap: 0 hrs primary care</span> · typical accepted range 40–100 total</p>
    </section>
  )
  if (category === 'clinical') return (
    <section className={shell}>
      ...
      <p className="flex flex-wrap items-center gap-2 text-xs font-semibold text-muted-foreground xl:ml-auto"><span><b className="text-foreground">8.2 hrs/wk</b> → <b className="text-foreground">≈812</b> by Jun 2027</span><span className="inline-flex items-center gap-1 text-emerald-600 dark:text-emerald-300"><ClipboardCheck className="size-3.5" /> clears your 750 target</span></p>
    </section>
  )
  return (
    <section className={shell}>
      <SummaryKpi value="2" label="labs / projects" /><SummaryKpi value="248" label="total hours" /><SummaryKpi value="5" label="outputs" />
      <div className="flex flex-wrap items-center gap-2 text-xs font-semibold xl:ml-auto"><StatusDot color="bg-muted-foreground" label="idea 1" /><StatusDot color="bg-primary" label="submitted 2" /><StatusDot color="bg-amber-500" label="accepted 1" /><StatusDot color="bg-emerald-500" label="presented 1" /></div>
    </section>
  )
}
        ...
        {entities.map((entity) => (
          <button key={entity.id} type="button" role="tab" aria-selected={entity.id === activeId} onClick={() => onSelect(entity.id)} className={cn('relative min-h-[70px] rounded-xl border bg-card px-3.5 py-3 text-left shadow-sm transition', entity.id === activeId ? 'ring-1 ring-current/25' : 'hover:border-primary/35')} style={entity.id === activeId ? { borderColor: pillarAccent(category), color: 'inherit' } : undefined}>
            <div className="flex items-start gap-2">
              ...
              <h2 className="min-w-0 flex-1 font-display text-sm font-semibold leading-tight">{entity.name}</h2>
              <strong className="font-display text-lg leading-none tabular-nums">{entity.hours.replace('h', '')}</strong>
            </div>
            <p className="mt-1.5 text-xs font-semibold leading-snug text-muted-foreground">{entity.subtitle}</p>
            <p className={cn('mt-1 text-xs font-semibold leading-snug', entity.tone === 'warn' ? 'text-amber-600 dark:text-amber-300' : 'text-muted-foreground')}>{entity.meta}</p>
          </button>
        ))}
        <button type="button" onClick={onAdd} className="grid min-h-[70px] place-items-center rounded-xl border border-dashed bg-muted/10 p-3 text-sm font-semibold text-muted-foreground hover:border-primary/40 hover:text-primary"><span>+ {addLabel.toLowerCase()}</span></button>
      </div>
    </section>
  )
}
...
function SectionShell({ title, hint, children, className }: { title: string; hint?: string; children: ReactNode; className?: string }) {
  return <section className={cn('overflow-hidden rounded-xl border bg-card px-4 py-4 shadow-sm', className)}><div className="mb-3"><h2 className="font-display text-sm font-semibold">{title}</h2>{hint && <p className="mt-0.5 text-xs leading-snug text-muted-foreground">{hint}</p>}</div>{children}</section>
}
...
function VerificationLedger() {
  ...
  return <SectionShell title="Verification ledger" hint="Formatted the way AMCAS asks for it. One copy button per organization at application time."><DataGrid headers={['Organization', 'Total hrs', 'Avg / wk', 'Dates', 'Contact (verifier)', 'Type', '']} mobileMinWidth="min-w-[47.5rem]" rows={rows.map((row) => [<span><strong>{row[0]}</strong><span className="block text-xs text-muted-foreground">{row[1]}</span></span>, row[2], row[3], row[4], <span><strong>{row[5]}</strong><span className="block text-xs text-muted-foreground">{row[6]}</span></span>, <StatusChip>{row[7]}</StatusChip>, <button className="inline-flex items-center gap-1 whitespace-nowrap text-xs font-semibold text-primary"><Copy className="size-3" /> Copy for AMCAS</button>])} /></SectionShell>
}
...
function SpecialtyExposure() {
  ...
  return <SectionShell title="Specialty exposure" hint="A count and list of specialties and settings, with the next gap called out plainly."><DataGrid headers={['Specialty', 'Hours', 'Share', 'Settings', 'Next move']} mobileMinWidth="min-w-[40rem]" rows={rows.map((row) => [<span className="font-semibold">{row[0]}</span>, row[1], <span className="text-muted-foreground">{row[2]}</span>, <span className="text-muted-foreground">{row[3]}</span>, <span className="font-semibold text-primary">{row[4]} →</span>])} /></SectionShell>
}
...
function ResearchOutputs() {
  ...
  return <SectionShell title="Outputs" hint="Every poster, abstract, talk, and paper as a row — the thing adcoms actually count."><DataGrid headers={['Project / output', 'Type', 'Venue', 'Deadline / milestone', 'Status']} mobileMinWidth="min-w-[45rem]" rows={rows.map((row) => [<span><strong>{row[0]}</strong><span className="block text-xs text-muted-foreground">{row[1]}</span></span>, row[2], row[3], row[4], <StatusChip tone={row[5] === 'presented' || row[5] === 'accepted' ? 'good' : row[5] === 'idea' ? 'quiet' : 'primary'}>{row[5]}</StatusChip>])} /></SectionShell>
}

function DataGrid({ headers, rows, mobileMinWidth }: { headers: string[]; rows: ReactNode[][]; mobileMinWidth: string }) {
  const columns = { gridTemplateColumns: `repeat(${headers.length}, minmax(0, 1fr))` }
  return <div className="overflow-x-auto"><div className={cn('text-sm font-semibold md:min-w-0', mobileMinWidth)}><div className="grid bg-muted/25 text-left text-xs uppercase tracking-wide text-muted-foreground" style={columns}>{headers.map((header) => <div key={header} className="px-3 py-2">{header}</div>)}</div>{rows.map((row, rowIndex) => <div key={rowIndex} className="grid border-t border-border/70" style={columns}>{row.map((cell, cellIndex) => <div key={cellIndex} className="min-w-0 px-3 py-2 align-middle">{cell}</div>)}</div>)}</div></div>
}

function StatusChip({ children, tone = 'primary' }: { children: ReactNode; tone?: 'primary' | 'good' | 'warn' | 'quiet' }) {
  return <span className={cn('inline-flex rounded-full px-2 py-0.5 text-xs font-semibold', tone === 'good' && 'bg-emerald-500/12 text-emerald-700 dark:text-emerald-300', tone === 'warn' && 'bg-amber-500/12 text-amber-700 dark:text-amber-300', tone === 'quiet' && 'bg-muted text-muted-foreground', tone === 'primary' && 'bg-primary/10 text-primary')}>{children}</span>
}

function WorkspaceHero({ title, metadata, hours, footer }: { title: string; metadata: ReactNode; hours: string; footer: ReactNode }) {
  return <div className="mb-3.5 grid gap-3 rounded-xl border bg-muted/20 px-4 py-4 sm:grid-cols-[1fr_auto]"><div className="min-w-0"><h2 className="font-display text-lg font-semibold leading-tight">{title}</h2><div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-sm font-semibold text-muted-foreground">{metadata}</div></div><div><strong className="font-display text-3xl font-semibold leading-none tabular-nums">{hours}</strong><span className="block text-xs font-semibold uppercase tracking-[0.06em] text-muted-foreground">hours here</span></div><div className="flex flex-wrap gap-x-3 gap-y-1 border-t border-dashed border-border pt-2 text-xs font-semibold text-muted-foreground sm:col-span-2">{footer}</div></div>
}
...
function WorkspaceModule({ title, icon: Icon, children, action, className }: { title: string; icon: ComponentType<{ className?: string }>; children: ReactNode; action?: string; className?: string }) {
  return <section className={cn('rounded-xl border bg-muted/15 px-4 py-3', className)}><div className="mb-2.5 flex items-center gap-2"><Icon className="size-3.5 text-primary" /><h3 className="text-xs font-semibold uppercase tracking-[0.08em]">{title}</h3>{action && <span className="ml-auto text-xs font-semibold text-primary">{action}</span>}</div>{children}</section>
}
...
function ResearchWorkspace({ entity, onAddEntry }: { entity: Entity; onAddEntry: AddExperience }) {
  return <section className="rounded-xl border bg-card p-4 shadow-sm">
    ...
    <div className="grid gap-3 lg:grid-cols-2"><WorkspaceModule title="Meetings with PI" icon={Users}><div className="rounded-lg bg-muted/25 p-2.5"><strong className="text-sm">Jul 10 · 1:1</strong><p className="mt-1 text-sm text-muted-foreground">Agreed I lead the SfN poster figures. She wants n bumped before submission.</p></div><p className="mt-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">To discuss next time</p><div className="mt-1.5 space-y-1.5">{['Ask about authorship order on the methods paper', 'Confocal time conflict w/ Tue clinicals', 'Summer stipend / work-study paperwork'].map((item) => <label key={item} className="flex items-center gap-2 text-sm"><input type="checkbox" className="size-3.5 rounded" />{item}</label>)}</div></WorkspaceModule><WorkspaceModule title="Lab notebook / hours" icon={Microscope}><ExactLogRows initialDate="Jul 14" rows={[['Jul 11', 'Confocal — 6 slices, GFAP+Iba1 co-stain', '5.0'], ['Jul 8', 'ImageJ morphology batch + QC', '4.0'], ['Jul 3', 'Perfusions + tissue collection (n=4)', '6.0']]} onAdd={(values) => onAddEntry({ org: entity.name, startDate: values[0], hours: Number(values[1]), description: values[2], status: 'active' })} placeholder="what you did in lab" /></WorkspaceModule></div>
  </section>
}
function SmallFact({ value, label }: { value: string; label: string }) { return <div className="rounded-lg bg-muted/25 px-2.5 py-2"><strong className="font-display text-lg leading-none">{value}</strong><span className="mt-1 block text-xs text-muted-foreground">{label}</span></div> }
function LabeledRow({ label, value, action }: { label: string; value: string; action: string }) { return <div className="grid gap-1 rounded-lg bg-muted/25 px-2.5 py-2 sm:grid-cols-[7rem_1fr_auto] sm:items-center"><strong className="text-sm">{label}</strong><span className="text-xs text-muted-foreground">{value}</span><span className="text-xs font-semibold text-primary">{action}</span></div> }
function SkillRow({ label, observed, performed }: { label: string; observed: string; performed: string }) { return <div className="grid grid-cols-[1fr_auto_auto] items-center gap-2 rounded-lg bg-muted/20 px-2.5 py-1.5 text-sm"><strong>{label}</strong><span className="text-xs text-muted-foreground">Observed {observed}</span><span className="text-xs font-semibold text-primary">Performed {performed}</span></div> }
...
function ExactLogRows({ rows, onAdd, placeholder, initialDate }: { rows: string[][]; onAdd: (values: string[]) => void; placeholder: string; initialDate: string }) {
  ...
  return <div><div>{rows.map((row) => {
    ...
    return <div key={`${row[0]}-${row[1]}`} className="grid gap-1.5 border-b border-dashed border-border px-1 py-2 sm:grid-cols-[4.5rem_1fr_3.25rem] sm:items-center"><strong className="text-xs text-muted-foreground">{row[0]}</strong><span className="text-sm"><span className="font-semibold">{main}</span>{detail && <span className="mt-0.5 block text-xs leading-snug text-muted-foreground">{detail}</span>}</span><strong className="text-right text-sm">{row[2]}h</strong></div>
