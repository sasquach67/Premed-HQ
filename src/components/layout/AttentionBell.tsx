    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="outline" size="icon" className="relative size-8 rounded-full bg-card/80" aria-label={`Attention${count ? `, ${count} important items` : ''}`}>
          ...
          {count > 0 && <span className="absolute -right-1 -top-1 grid min-w-4 place-items-center rounded-full bg-destructive px-1 text-[9px] font-semibold leading-4 text-destructive-foreground">{count > 9 ? '9+' : count}</span>}
        </Button>
      </PopoverTrigger>
      <PopoverContent align="end" className="w-[min(25rem,calc(100vw-1rem))] p-0 max-sm:fixed max-sm:bottom-2 max-sm:left-2 max-sm:right-2 max-sm:top-auto max-sm:w-auto">
        <div className="border-b border-border px-4 py-3">
          <p className="font-display text-base font-semibold">Attention</p>
          <p className="text-xs text-muted-foreground">Deadlines that deserve a decision now.</p>
        </div>
        <ScrollArea className="h-[min(60vh,30rem)]">
          <div className="p-2">
          {!items.length && <div className="px-4 py-8 text-center"><Bell className="mx-auto size-6 text-primary" /><p className="mt-2 text-sm font-semibold">Nothing needs attention</p><p className="mt-1 text-xs text-muted-foreground">You’re clear for now.</p></div>}
          {items.map((item) => (
