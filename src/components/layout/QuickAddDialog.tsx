        ...
        <DialogContent className="max-h-[90svh] overflow-y-auto sm:max-w-xl">
          ...
          {!activeKind ? (
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              {(activeKind === 'task' || activeKind === 'assignment' || activeKind === 'hours' || activeKind === 'mistake') && <div className="space-y-1.5"><Label htmlFor="quick-date">{activeKind === 'hours' ? 'Date' : 'Due date'}</Label><Input id="quick-date" type="date" value={date} onChange={(event) => setDate(event.target.value)} /></div>}
